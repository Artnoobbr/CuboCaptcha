...
async def on_member_join(context):
    ...
    img.save(nome)
    embed = discord.Embed(title=f'Olá! digite o que está na imagem para ser verificado no {guild}!', description='Voce tem 30 sec pra terminar', timestamp=datetime.utcnow())
    embed.set_footer(text='Cubo Captcha ©2021')
    await user.send(embed=embed, file= discord.File(nome))
    def check(m):
